import time

from AttendanceSystem.Person import Person
from SqlController import sqlController


class Manager:
    def __init__(self):
        pass

    def AddPerson(self, _name, _id) -> Person:  # 添加人员, 并返回添加的Person对象
        p = Person(_name, _id)
        p.SetFaceInfo()
        sqlController.InsertPerson(p.id, p.name, p.record)
        return p

    def DeletePerson(self, _id):
        p = self.GetPerson(_id)
        if p is None:
            return
        sqlController.DeletePerson(p.id)
        sqlController.Delete(p.name)

    def UpdatePerson(self, _id, _name, _record: dict):
        sqlController.UpdatePerson(_id, _name, _record)

    def GetPerson(self, _id: str):
        res = sqlController.SelectPerson(_id)
        if res is None:
            return None
        return Person(res[0], _id, res[1])


manager = Manager()


def test1():
    p = manager.GetPerson('002')
    p.printInfo()
    p.name = 'ZhangSan'
    p.SignIn()
    time.sleep(3)
    p.SignOut()
    manager.UpdatePerson(p.id, p.name, p.record)
    p = manager.GetPerson('002')
    p.printInfo()


def test2():
    p = manager.AddPerson('LiSi', '001')
    p.printInfo()
    manager.DeletePerson('001')
    p = manager.GetPerson('001')
    if p is None:
        print('删除成功')
    else:
        p.printInfo()


if __name__ == '__main__':
    test2()
